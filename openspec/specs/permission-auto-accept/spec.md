...
The `wait_for_response_file()` polling loop SHALL check for opt-in auto-accept handling when `waiting_user_answer` is detected, before the file-exists and idle-grace checks. It SHALL either auto-accept (if opted in) or log a warning (if not), rather than silently treating it as processing.
